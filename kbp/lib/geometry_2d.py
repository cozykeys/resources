...
class Vector2D(SvgWritable):
    ...
    def add(self, rhs: "Vector2D") -> "Vector2D":
...
class Line2D(SvgWritable):
    ...
    def get_x_at_y(self, y) -> float:
        # Convert to Slope Intercept Form: y = mx + b
        m = self.direction.slope()

        # I'm sure there's a more efficient way to do this but I'm being lazy
        segment = Segment2D(self.point, self.point.add(self.direction))
        y_axis = Segment2D(Vector2D(0.0, 0.0), Vector2D(0.0, 1.0))
        p = segment.intersection(y_axis)
        b = p.y
