...
def print_switch_to_diode_segment(key):
    ...
    print_segment(start_x, start_y, end_x, end_y, net_id, "Back")
...
def main():
    ...
    for key in keys:
        ...
        if not row in key_dict:
            key_dict[row] = {}
        key_dict[row][column] = key

        print_switch_to_diode_segment(key)
        print_switch_to_column_segment(key)

    print_column_segments()

    for r in range(0,5):
        for c in range(0,5):
            print_switch_pair_row_segments_left(key_dict[r][c], key_dict[r][c+1])
    print_switch_pair_row_segments_left(key_dict[4][5], key_dict[0][6])

    for r in range(0,5):
        for c in range(12,7,-1):
            print_switch_pair_row_segments_right(key_dict[r][c], key_dict[r][c-1])
    print_switch_pair_row_segments_right(key_dict[4][7], key_dict[1][6])
