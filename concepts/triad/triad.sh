...
triad_dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"

# TODO: Is there a better way? Submodule perhaps?
kbutil_dir="$( dirname "$triad_dir" )/kbutil"
...
function generate_render() {
    input="$triad_dir/triad.xml"
    output="$triad_dir/case"
    ...
    "$svg_opener" "$output/triad.svg"
}

function generate_ponoko() {
    input="$triad_dir/triad.xml"
    output="$triad_dir/case/ponoko"

    dotnet "$kbutil_dll" gen-svg $options "$input" "$output"

    "$svg_opener" "$output/triad_Switch.svg" 
}
...
function generate_pcb() {
    pcb_dir="$triad_dir/pcb"

    input="$triad_dir/switches.json"
    output="$pcb_dir/triad.kicad_pcb"
    ...
    mkdir -p "$pcb_dir"
    dotnet "$kbutil_dll" gen-pcb "triad" "$input" "$output"
}
...
function print_usage() {
    echo "USAGE: ./triad.sh <action> [OPTIONS]"
    echo ""
    echo "Actions:"
